...
int NinjaMain::ToolRules(const Options* options, int argc, char* argv[]) {
  ...
  int opt;
  // GGBUILD
  while ((opt = getopt_ninja(argc, argv, const_cast<char*>("hd"))) != -1) {
  // GGBUILD
    switch (opt) {
    case 'd':
...
int NinjaMain::ToolCommands(const Options* options, int argc, char* argv[]) {
  ...
  int opt;
  // GGBUILD
  while ((opt = getopt_ninja(argc, argv, const_cast<char*>("hs"))) != -1) {
  // GGBUILD
    switch (opt) {
    case 's':
      mode = PCM_Single;
...
int NinjaMain::ToolClean(const Options* options, int argc, char* argv[]) {
  ...
  int opt;
  // GGBUILD
  while ((opt = getopt_ninja(argc, argv, const_cast<char*>("hgr"))) != -1) {
  // GGBUILD
    switch (opt) {
    case 'g':
      generator = true;
...
int NinjaMain::ToolCompilationDatabase(const Options* options, int argc,
  ...
  int opt;
  // GGBUILD
  while ((opt = getopt_ninja(argc, argv, const_cast<char*>("hx"))) != -1) {
  // GGBUILD
    switch(opt) {
      case 'x':
...
int NinjaMain::ToolRestat(const Options* options, int argc, char* argv[]) {
  ...
  int opt;
  // GGBUILD
  while ((opt = getopt_ninja(argc, argv, const_cast<char*>("h"))) != -1) {
  // GGBUILD
    switch (opt) {
    case 'h':
    default:
